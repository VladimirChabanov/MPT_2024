...
1. Создайте две виртуальные машины с Ubuntu Server . В процессе установки используйте стандартные настройки. Из дополнительного софта понадобится только ssh-сервер.  
...
Установим Ansible согласно инструкции с [оф сайта](https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html#installing-and-upgrading-ansible-with-pipx). Absible написан на Python, поэтому его можно установить как обычный Python-пакет.
В зависимости от версии Python (в новых версиях) во время установки Ansible через pip можно получить ошибку связанную с тем, что pip откажется устанавливать пакет в глобальное окружение ([PEP 668](https://peps.python.org/pep-0668/)), а если установить его в виртуальное окружение, то каждый раз для использования Ansible придётся это окружение активировать. Чтобы исправить ситуацию, воспользуемся пакетом [`pipx`](https://pipx.pypa.io/stable/), который автоматически создаёт изолированные окружения для Python-приложений.
...
8. Проверьте, что к этой машине можно подключиться как и к любой другой по ssh (ip, port и пользователь вам известны).
...
5. Проверьте, что машины доступны при помощи модуля ansible `ping`. Прошпигуйте все воркеры и только те, которые находятся в группе "prod" используя имена групп.
...
4. Запустите palybook "setup.yaml" и дождитесь его завершения. В логах вы должны увидеть, что на всех серверах кроме "worker4" на каждом этапе происходили изменения (changed), а на "worker4" изменений не было (ok), т.к. мы выполнили указанные действия вручную в предыдущем разделе.
...
4. Добавьте новые сервера в файлы "hosts_root" и "host".

5. Прошпигуйте сервера указав в качестве инвентаря "hosts_root" (т.к. сервера сброшены в начальное состояние).
...
7. Подождите несколько минут и прошпигуйте сервера указав в качестве инвентаря "hosts".

8. Запустите установку всего необходимого софта для сервера базы данных и воркеров указав в качестве инвентаря "hosts".

9. В файле "install_haproxy.yaml", в конфиге балансировщика, в раздел "backend" добавьте новые рабочие сервера и тоже запустите установку.

10. Протестируйте работоспособность приложения, можно просто обновить вкладку раз 20-30.

11. Зайдите на страницу "/haproxy_stats" и проверьте, что все сервера работают и загружены равномерно.
